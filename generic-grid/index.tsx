...
import BaseGrid from "../_shared/BaseGrid"
...
const GenericGrid: QuartzComponent = ({ allFiles, fileData, options }: QuartzComponentProps & { options?: Options }) => {
  ...
  const pages = allFiles.filter((page) => {
    ...
    // 3. Application:
    // If it's books, ONLY show things that are NOT finished (i.e. this is
    // the "currently reading / recommendations" list, not the read list).
